...
CREATE TABLE IF NOT EXISTS `orders` (
  ...
  `refund_status` enum('none','requested','approved','processed','rejected') DEFAULT 'none',
...
CREATE TABLE IF NOT EXISTS `products` (
  ...
  `status` enum('active','inactive','pending','out_of_stock','phase_out','unavailable','approved','rejected') DEFAULT 'active',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `returns_inbound` (
  `id` int(11) NOT NULL,
  `order_id` int(11) DEFAULT NULL,
  `product_id` int(11) NOT NULL,
  `warehouse_id` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `subscription_packages` (
  ...
  `validity_date` enum('Monthly','Quarterly','Yearly') DEFAULT 'Monthly',
...
--
CREATE TABLE IF NOT EXISTS `v_delivery_daily` (
`period` date
...
-- (See below for the actual view)
--
CREATE TABLE IF NOT EXISTS `v_inventory_low_stock` (
`id` int(11)
,`name` varchar(255)
...
-- (See below for the actual view)
--
CREATE TABLE IF NOT EXISTS `v_order_status_daily` (
`period` date
...
-- (See below for the actual view)
--
CREATE TABLE IF NOT EXISTS `v_sales_daily` (
`period` date
,`orders` bigint(21)
,`units` decimal(32,0)
,`gross` decimal(42,2)
...
CREATE OR REPLACE VIEW `v_order_status_daily` AS SELECT cast(`o`.`created_at` as date) AS `period`, `o`.`status` AS `status`, count(0) AS `count` FROM `orders` AS `o` GROUP BY cast(`o`.`created_at` as date), `o`.`status` ;
...
(1, 'Employee Directory', 'directory', 'Complete employee directory with contact information', 'SELECT u.*, d.name as department_name FROM employees u LEFT JOIN departments d ON u.department_id = d.id WHERE u.status = \"active\"', '[]', 1, 1, '2025-09-21 09:02:58', '2025-09-21 09:02:58'),
(2, 'Leave Summary Report', 'leave', 'Summary of leave balances and usage by employee', 'SELECT u.first_name, u.last_name, lt.name as leave_type, lb.allocated_days, lb.used_days, lb.remaining_days FROM employees u JOIN leave_balances lb ON u.id = lb.employee_id JOIN leave_types lt ON lb.leave_type_id = lt.id WHERE lb.year = ?', '[{\"name\": \"year\", \"type\": \"number\", \"required\": true}]', 1, 1, '2025-09-21 09:02:58', '2025-09-21 09:02:58'),
(3, 'Payroll Summary Report', 'payroll', 'Monthly payroll summary by department', 'SELECT d.name as department, COUNT(ps.employee_id) as employee_count, SUM(ps.gross_pay) as total_gross, SUM(ps.total_deductions) as total_deductions, SUM(ps.net_pay) as total_net FROM payroll_summaries ps JOIN employees u ON ps.employee_id = u.id JOIN departments d ON u.department_id = d.id WHERE ps.payroll_run_id = ? GROUP BY d.id, d.name', '[{\"name\": \"payroll_run_id\", \"type\": \"number\", \"required\": true}]', 1, 1, '2025-09-21 09:02:58', '2025-09-21 09:02:58');
...
INSERT INTO `employees` (`id`, `employee_id`, `email`, `password`, `first_name`, `last_name`, `middle_name`, `phone`, `address`, `date_of_birth`, `gender`, `position`, `department_id`, `manager_id`, `employment_type`, `start_date`, `salary`, `status`, `role`, `created_at`, `updated_at`) VALUES
...
(7, 'EMP20250003', 'mike.johnson@company.com', '$2y$10$D3UWeg41.dGCvFNU7FSHp.n20IKKUPcAdYpYO.ZMxOVX8zIrrNjA6', 'Mike', 'Johnson', NULL, '5550103', NULL, NULL, NULL, 'Software Developer', 1, NULL, 'Full-time', '2025-09-21', 42500.00, 'active', 'employee', '2025-09-21 01:46:55', '2025-09-21 01:46:55');
...
--
ALTER TABLE `competency_assessments`
  ADD CONSTRAINT `competency_assessments_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `competency_assessments_ibfk_2` FOREIGN KEY (`competency_id`) REFERENCES `competencies` (`id`),
  ADD CONSTRAINT `competency_assessments_ibfk_3` FOREIGN KEY (`assessor_id`) REFERENCES `employees` (`id`);

--
-- Constraints for table `courses`
--
ALTER TABLE `courses`
  ADD CONSTRAINT `courses_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `employees` (`id`);

--
-- Constraints for table `course_enrollments`
--
ALTER TABLE `course_enrollments`
  ADD CONSTRAINT `course_enrollments_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `course_enrollments_ibfk_2` FOREIGN KEY (`course_id`) REFERENCES `courses` (`id`),
  ADD CONSTRAINT `fk_enrolled_by` FOREIGN KEY (`enrolled_by`) REFERENCES `employees` (`id`);
...
ALTER TABLE `employee_documents`
  ...
  ADD CONSTRAINT `employee_documents_ibfk_2` FOREIGN KEY (`uploaded_by`) REFERENCES `employees` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `employee_skills`
  ...
  ADD CONSTRAINT `employee_skills_ibfk_2` FOREIGN KEY (`verified_by`) REFERENCES `employees` (`id`) ON DELETE SET NULL;

--
-- Constraints for table `expense_claims`
--
ALTER TABLE `expense_claims`
  ADD CONSTRAINT `expense_claims_ibfk_1` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `expense_claims_ibfk_2` FOREIGN KEY (`approved_by`) REFERENCES `employees` (`id`) ON DELETE SET NULL;
...
--
ALTER TABLE `hr_reports`
  ADD CONSTRAINT `hr_reports_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `employees` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `job_applications`
  ...
  ADD CONSTRAINT `job_applications_ibfk_2` FOREIGN KEY (`referral_id`) REFERENCES `employees` (`id`);
...
ALTER TABLE `job_postings`
  ...
  ADD CONSTRAINT `job_postings_ibfk_2` FOREIGN KEY (`hiring_manager_id`) REFERENCES `employees` (`id`);
...
ALTER TABLE `onboarding`
  ...
  ADD CONSTRAINT `onboarding_ibfk_2` FOREIGN KEY (`buddy_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `onboarding_ibfk_3` FOREIGN KEY (`mentor_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `onboarding_ibfk_4` FOREIGN KEY (`created_by`) REFERENCES `employees` (`id`);
...
ALTER TABLE `payroll_runs`
  ...
  ADD CONSTRAINT `payroll_runs_ibfk_2` FOREIGN KEY (`processed_by`) REFERENCES `employees` (`id`) ON DELETE SET NULL;
...
ALTER TABLE `payroll_summaries`
  ...
  ADD CONSTRAINT `payroll_summaries_ibfk_2` FOREIGN KEY (`employee_id`) REFERENCES `employees` (`id`) ON DELETE CASCADE;
...
ALTER TABLE `performance_reviews`
  ...
  ADD CONSTRAINT `performance_reviews_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `employees` (`id`);

--
-- Constraints for table `recognition`
--
ALTER TABLE `recognition`
  ADD CONSTRAINT `recognition_ibfk_1` FOREIGN KEY (`recipient_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `recognition_ibfk_2` FOREIGN KEY (`giver_id`) REFERENCES `employees` (`id`);
...
ALTER TABLE `salary_adjustments`
  ...
  ADD CONSTRAINT `salary_adjustments_ibfk_2` FOREIGN KEY (`approved_by`) REFERENCES `employees` (`id`) ON DELETE SET NULL;
...
ALTER TABLE `succession_candidates`
  ...
  ADD CONSTRAINT `succession_candidates_ibfk_2` FOREIGN KEY (`candidate_id`) REFERENCES `employees` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `succession_candidates_ibfk_3` FOREIGN KEY (`added_by`) REFERENCES `employees` (`id`);
...
ALTER TABLE `succession_plans`
  ...
  ADD CONSTRAINT `succession_plans_ibfk_2` FOREIGN KEY (`current_incumbent_id`) REFERENCES `employees` (`id`),
  ADD CONSTRAINT `succession_plans_ibfk_3` FOREIGN KEY (`created_by`) REFERENCES `employees` (`id`);
...
ALTER TABLE `timesheets`
  ...
  ADD CONSTRAINT `timesheets_ibfk_2` FOREIGN KEY (`approved_by`) REFERENCES `employees` (`id`) ON DELETE SET NULL;
...
-- Note: users table constraints removed - users table doesn't exist in hr_core database
-- Only employees table exists in hr_core, which already has proper constraints
-- =====================
...
CREATE TABLE IF NOT EXISTS `budget_requests` (
  ...
  KEY `idx_br_requested_by` (`requested_by`),
  KEY `idx_br_approved_by` (`approved_by`),
  KEY `idx_br_request_date` (`request_date`),
  KEY `idx_br_fiscal_year` (`fiscal_year`),
  CONSTRAINT `fk_br_department` FOREIGN KEY (`department_id`) REFERENCES `departments` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_br_requested_by` FOREIGN KEY (`requested_by`) REFERENCES `users` (`id`) ON DELETE RESTRICT,
...
-- ============================
-- MISSING TABLES FROM FINANCIAL_DATABASE_SCHEMA.SQL
-- ============================
...
CREATE TABLE IF NOT EXISTS `disbursement_requests` (
  ...
  `payment_method` enum('Cash','Check','Bank Transfer','Wire Transfer') DEFAULT 'Bank Transfer',
...
CREATE TABLE IF NOT EXISTS `disbursement_approvals` (
  ...
  CONSTRAINT `fk_da_request` FOREIGN KEY (`request_id`) REFERENCES `disbursement_requests` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `collection_schedules` (
  ...
  `status` enum('Scheduled','Collected','Cancelled','Overdue') DEFAULT 'Scheduled',
  ...
  CONSTRAINT `fk_cs_invoice` FOREIGN KEY (`invoice_id`) REFERENCES `sales_invoices` (`id`) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `collection_activities` (
  ...
  `activity_type` enum('Call','Visit','Email','Letter','Other') DEFAULT 'Call',
  `activity_date` timestamp NOT NULL DEFAULT current_timestamp(),
  `activity_notes` text DEFAULT NULL,
  `result` enum('Successful','Unsuccessful','Rescheduled','Cancelled') DEFAULT 'Unsuccessful',
  `collected_amount` decimal(15,2) DEFAULT 0.00,
  `performed_by` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_schedule` (`schedule_id`),
  KEY `idx_activity_date` (`activity_date`),
  CONSTRAINT `fk_ca_schedule` FOREIGN KEY (`schedule_id`) REFERENCES `collection_schedules` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
-- ============================
-- MISSING TABLES FROM ECOMMERCE_FINANCIAL.SQL
-- ============================
...
CREATE TABLE IF NOT EXISTS `accounts_payable` (
  ...
  `status` enum('pending','approved','paid','overdue') DEFAULT 'pending',
  ...
  KEY `idx_accounts_payable_due` (`due_date`),
  CONSTRAINT `accounts_payable_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `accounts_receivable` (
  ...
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  KEY `idx_accounts_receivable_due` (`due_date`),
  CONSTRAINT `accounts_receivable_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `audit_trail` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `idx_audit_trail_table_record` (`table_name`,`record_id`),
  KEY `idx_audit_trail_user` (`user_id`),
...
CREATE TABLE IF NOT EXISTS `budget_approval_workflow` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `request_id` int(11) NOT NULL,
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `idx_budget_approval_workflow_request` (`request_id`),
  KEY `idx_budget_approval_workflow_approver` (`approver_id`),
  CONSTRAINT `budget_approval_workflow_ibfk_1` FOREIGN KEY (`request_id`) REFERENCES `budget_requests` (`id`) ON DELETE CASCADE,
  CONSTRAINT `budget_approval_workflow_ibfk_2` FOREIGN KEY (`approver_id`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `budget_management` (
  ...
  `budget_type` enum('operational','capital','marketing','hr','other') NOT NULL,
  `allocated_amount` decimal(15,2) NOT NULL,
  `spent_amount` decimal(15,2) DEFAULT 0.00,
  `remaining_amount` decimal(15,2) GENERATED ALWAYS AS (`allocated_amount` - `spent_amount`) VIRTUAL,
  ...
  CONSTRAINT `budget_management_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Commission Rates (must be defined before collections)
CREATE TABLE IF NOT EXISTS `commission_rates` (
  ...
  `applicable_types` text DEFAULT NULL,
  `is_active` tinyint(1) DEFAULT 1,
  `created_by` int(11) DEFAULT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  CONSTRAINT `commission_rates_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Payment Platforms (must be defined before collections)
CREATE TABLE IF NOT EXISTS `payment_platforms` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `platform_name` varchar(100) NOT NULL,
  `platform_code` varchar(20) NOT NULL,
  `platform_type` enum('e_wallet','bank','credit_card','cash','other') NOT NULL,
  `is_active` tinyint(1) DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `collections` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `collection_type` enum('payment','invoice','commission','subscription') NOT NULL,
  `amount` decimal(15,2) NOT NULL,
  ...
  `commission_status` enum('not_calculated','calculated','approved','paid') DEFAULT 'not_calculated',
  `commission_notes` text DEFAULT NULL,
  `platform_id` int(11) DEFAULT NULL,
  `platform_transaction_id` varchar(100) DEFAULT NULL,
  `platform_reference` varchar(100) DEFAULT NULL,
  `reconciliation_status` enum('pending','matched','unmatched','disputed') DEFAULT 'pending',
  `receipt_generated` tinyint(1) DEFAULT 0,
  `region` varchar(100) DEFAULT NULL,
  `auto_reconciled` tinyint(1) DEFAULT 0,
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  KEY `idx_collections_date` (`collection_date`),
  KEY `idx_collections_status` (`status`),
  KEY `fk_collections_commission_rate` (`commission_rate_id`),
  KEY `idx_collections_commission_status` (`commission_status`),
  KEY `idx_collections_platform` (`platform_id`),
  KEY `idx_collections_reconciliation` (`reconciliation_status`),
  KEY `idx_collections_region` (`region`),
  CONSTRAINT `collections_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`),
  CONSTRAINT `fk_collections_commission_rate` FOREIGN KEY (`commission_rate_id`) REFERENCES `commission_rates` (`id`),
  CONSTRAINT `fk_collections_platform` FOREIGN KEY (`platform_id`) REFERENCES `payment_platforms` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `collection_analytics` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_date_platform` (`analytics_date`,`platform_id`),
  KEY `platform_id` (`platform_id`),
  KEY `idx_collection_analytics_date` (`analytics_date`),
  CONSTRAINT `collection_analytics_ibfk_1` FOREIGN KEY (`platform_id`) REFERENCES `payment_platforms` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `collection_receipts` (
  ...
  KEY `idx_collection_receipts_date` (`receipt_date`),
  CONSTRAINT `collection_receipts_ibfk_1` FOREIGN KEY (`collection_id`) REFERENCES `collections` (`id`) ON DELETE CASCADE,
  CONSTRAINT `collection_receipts_ibfk_2` FOREIGN KEY (`generated_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Collection Refunds
CREATE TABLE IF NOT EXISTS `collection_refunds` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `refund_number` varchar(50) NOT NULL,
  `collection_id` int(11) NOT NULL,
  `refund_amount` decimal(15,2) NOT NULL,
  `refund_reason` enum('customer_request','dispute','error','fraud','other') NOT NULL,
  ...
  KEY `idx_collection_refunds_status` (`refund_status`),
  CONSTRAINT `collection_refunds_ibfk_1` FOREIGN KEY (`collection_id`) REFERENCES `collections` (`id`),
  CONSTRAINT `collection_refunds_ibfk_2` FOREIGN KEY (`processed_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `commission_calculations` (
  ...
  KEY `created_by` (`created_by`),
  KEY `idx_commission_calculations_collection` (`collection_id`),
  KEY `idx_commission_calculations_status` (`status`),
  KEY `idx_commission_calculations_date` (`calculation_date`),
  CONSTRAINT `commission_calculations_ibfk_1` FOREIGN KEY (`collection_id`) REFERENCES `collections` (`id`) ON DELETE CASCADE,
  CONSTRAINT `commission_calculations_ibfk_2` FOREIGN KEY (`commission_rate_id`) REFERENCES `commission_rates` (`id`),
  CONSTRAINT `commission_calculations_ibfk_3` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `commission_payments` (
  ...
  CONSTRAINT `commission_payments_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `commission_payment_details` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_payment_calculation` (`payment_id`,`calculation_id`),
  KEY `calculation_id` (`calculation_id`),
  CONSTRAINT `commission_payment_details_ibfk_1` FOREIGN KEY (`payment_id`) REFERENCES `commission_payments` (`id`) ON DELETE CASCADE,
  CONSTRAINT `commission_payment_details_ibfk_2` FOREIGN KEY (`calculation_id`) REFERENCES `commission_calculations` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `financial_periods` (
  ...
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  PRIMARY KEY (`id`),
  UNIQUE KEY `unique_period` (`period_name`,`start_date`,`end_date`),
  KEY `closed_by` (`closed_by`),
  KEY `idx_financial_periods_dates` (`start_date`,`end_date`),
  CONSTRAINT `financial_periods_ibfk_1` FOREIGN KEY (`closed_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `financial_statement_templates` (
  ...
  `template_type` enum('balance_sheet','income_statement','cash_flow') NOT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `created_by` (`created_by`),
  CONSTRAINT `financial_statement_templates_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `payment_reconciliations` (
  ...
  KEY `idx_payment_reconciliations_status` (`reconciliation_status`),
  CONSTRAINT `payment_reconciliations_ibfk_1` FOREIGN KEY (`collection_id`) REFERENCES `collections` (`id`) ON DELETE CASCADE,
  CONSTRAINT `payment_reconciliations_ibfk_2` FOREIGN KEY (`reconciled_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `procurement` (
  ...
  `quantity` int(11) NOT NULL,
  `unit_price` decimal(10,2) NOT NULL,
  `total_amount` decimal(15,2) GENERATED ALWAYS AS (`quantity` * `unit_price`) VIRTUAL,
  ...
  KEY `created_by` (`created_by`),
  CONSTRAINT `procurement_ibfk_1` FOREIGN KEY (`budget_id`) REFERENCES `budget_management` (`id`),
  CONSTRAINT `procurement_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
INSERT INTO `tax_rates` (`tax_code`, `tax_name`, `tax_rate`, `effective_date`) VALUES
('VAT', 'Value Added Tax', 12.00, '2024-01-01'),
('WHT', 'Withholding Tax', 2.00, '2024-01-01'),
('EXEMPT', 'Tax Exempt', 0.00, '2024-01-01');

-- Insert sample branches
INSERT INTO `branches` (`id`, `branch_code`, `branch_name`, `branch_address`, `branch_manager`, `is_active`, `created_at`) VALUES
(1, 'HQ', 'Headquarters', 'Makati City, Metro Manila', 'John Smith', 1, NOW()),
(2, 'BGC', 'BGC Branch', 'Bonifacio Global City, Taguig', 'Maria Garcia', 1, NOW()),
(3, 'ORT', 'Ortigas Branch', 'Ortigas Center, Pasig', 'Robert Johnson', 1, NOW()),
(4, 'ALB', 'Alabang Branch', 'Alabang, Muntinlupa', 'Sarah Wilson', 1, NOW());

-- Insert sample payment platforms
INSERT INTO `payment_platforms` (`id`, `platform_name`, `platform_code`, `platform_type`, `is_active`, `created_at`) VALUES
(1, 'GCash', 'GCASH', 'e_wallet', 1, NOW()),
(2, 'PayMongo', 'PAYMONGO', 'e_wallet', 1, NOW()),
(3, 'Maya', 'MAYA', 'e_wallet', 1, NOW()),
(4, 'PayPal', 'PAYPAL', 'e_wallet', 1, NOW()),
(5, 'BDO Online', 'BDO', 'bank', 1, NOW()),
(6, 'BPI Online', 'BPI', 'bank', 1, NOW()),
(7, 'Metrobank Online', 'METROBANK', 'bank', 1, NOW()),
(8, 'Credit Card', 'CREDIT_CARD', 'credit_card', 1, NOW()),
(9, 'Cash on Delivery', 'COD', 'cash', 1, NOW()),
(10, 'Bank Transfer', 'BANK_TRANSFER', 'bank', 1, NOW());

-- Insert sample currencies
INSERT INTO `currencies` (`id`, `currency_code`, `currency_name`, `symbol`, `exchange_rate`, `is_base_currency`, `is_active`, `updated_at`) VALUES
(1, 'PHP', 'Philippine Peso', '₱', 1.0000, 1, 1, NOW()),
(2, 'USD', 'US Dollar', '$', 56.5000, 0, 1, NOW()),
(3, 'EUR', 'Euro', '€', 61.2000, 0, 1, NOW()),
(4, 'JPY', 'Japanese Yen', '¥', 0.3800, 0, 1, NOW());

-- Insert sample commission rates
INSERT INTO `commission_rates` (`id`, `rate_name`, `commission_type`, `rate_value`, `min_amount`, `max_amount`, `applicable_types`, `is_active`, `created_by`, `created_at`, `updated_at`) VALUES
(1, 'Standard Sales Commission', 'percentage', 5.0000, 0.00, NULL, '["payment", "invoice"]', 1, 1, NOW(), NOW()),
(2, 'Subscription Commission', 'percentage', 3.0000, 0.00, NULL, '["subscription"]', 1, 1, NOW(), NOW()),
(3, 'High Value Commission', 'percentage', 7.5000, 0.00, NULL, '["payment", "invoice"]', 1, 1, NOW(), NOW()),
(4, 'Referral Bonus', 'fixed', 2500.0000, 0.00, NULL, '["commission"]', 1, 1, NOW(), NOW());

-- Insert sample financial periods
INSERT INTO `financial_periods` (`id`, `period_name`, `period_type`, `start_date`, `end_date`, `status`, `closed_by`, `closed_at`, `created_at`) VALUES
(1, 'January 2024', 'monthly', '2024-01-01', '2024-01-31', 'open', NULL, NULL, NOW()),
(2, 'February 2024', 'monthly', '2024-02-01', '2024-02-29', 'open', NULL, NULL, NOW()),
(3, 'March 2024', 'monthly', '2024-03-01', '2024-03-31', 'open', NULL, NULL, NOW()),
(4, 'April 2024', 'monthly', '2024-04-01', '2024-04-30', 'open', NULL, NULL, NOW()),
(5, 'May 2024', 'monthly', '2024-05-01', '2024-05-31', 'open', NULL, NULL, NOW()),
(6, 'June 2024', 'monthly', '2024-06-01', '2024-06-30', 'open', NULL, NULL, NOW()),
(7, 'July 2024', 'monthly', '2024-07-01', '2024-07-31', 'open', NULL, NULL, NOW()),
(8, 'August 2024', 'monthly', '2024-08-01', '2024-08-31', 'open', NULL, NULL, NOW()),
(9, 'September 2024', 'monthly', '2024-09-01', '2024-09-30', 'open', NULL, NULL, NOW()),
(10, 'October 2024', 'monthly', '2024-10-01', '2024-10-31', 'open', NULL, NULL, NOW()),
(11, 'November 2024', 'monthly', '2024-11-01', '2024-11-30', 'open', NULL, NULL, NOW()),
(12, 'December 2024', 'monthly', '2024-12-01', '2024-12-31', 'open', NULL, NULL, NOW()),
(13, 'Q1 2024', 'quarterly', '2024-01-01', '2024-03-31', 'open', NULL, NULL, NOW()),
(14, 'Q2 2024', 'quarterly', '2024-04-01', '2024-06-30', 'open', NULL, NULL, NOW()),
(15, 'Q3 2024', 'quarterly', '2024-07-01', '2024-09-30', 'open', NULL, NULL, NOW()),
(16, 'Q4 2024', 'quarterly', '2024-10-01', '2024-12-31', 'open', NULL, NULL, NOW()),
(17, '2024', 'yearly', '2024-01-01', '2024-12-31', 'open', NULL, NULL, NOW());

-- Insert sample budget management records
INSERT INTO `budget_management` (`id`, `budget_name`, `budget_type`, `allocated_amount`, `spent_amount`, `fiscal_year`, `quarter`, `status`, `created_by`, `created_at`) VALUES
(1, 'IT Infrastructure Budget', 'capital', 500000.00, 125000.00, 2024, 'Q1', 'active', 1, NOW()),
(2, 'Marketing Campaign Budget', 'marketing', 300000.00, 75000.00, 2024, 'Q1', 'active', 1, NOW()),
(3, 'HR Training Budget', 'hr', 150000.00, 25000.00, 2024, 'Q1', 'active', 1, NOW()),
(4, 'Operations Budget', 'operational', 1000000.00, 200000.00, 2024, 'Q1', 'active', 1, NOW());

-- Insert sample budget requests
INSERT INTO `budget_requests` (`id`, `request_number`, `department_id`, `budget_category`, `requested_amount`, `justification`, `request_date`, `requested_by`, `status`, `approved_amount`, `approved_by`, `approved_date`, `rejection_reason`, `fiscal_year`, `quarter`, `priority`, `attachments`, `created_at`, `updated_at`) VALUES
(1, 'BR-2024-001', 1, 'Software Licenses', 50000.00, 'Annual renewal of accounting and financial software licenses', '2024-01-15', 1, 'approved', 50000.00, 1, '2024-01-20', NULL, '2024', 'Q1', 'high', NULL, NOW(), NOW()),
(2, 'BR-2024-002', 2, 'Training Programs', 75000.00, 'Employee development and certification programs', '2024-01-16', 1, 'pending', 0.00, NULL, NULL, NULL, '2024', 'Q1', 'medium', NULL, NOW(), NOW()),
(3, 'BR-2024-003', 3, 'Hardware Upgrade', 150000.00, 'Server and workstation upgrades for improved performance', '2024-01-17', 1, 'approved', 120000.00, 1, '2024-01-22', NULL, '2024', 'Q1', 'critical', NULL, NOW(), NOW()),
(4, 'BR-2024-004', 4, 'Marketing Campaign', 200000.00, 'Digital marketing campaign for Q1 product launch', '2024-01-18', 1, 'pending', 0.00, NULL, NULL, NULL, '2024', 'Q1', 'high', NULL, NOW(), NOW()),
(5, 'BR-2024-005', 5, 'Sales Tools', 30000.00, 'CRM software and sales automation tools', '2024-01-19', 1, 'approved', 30000.00, 1, '2024-01-25', NULL, '2024', 'Q1', 'medium', NULL, NOW(), NOW());

-- Insert sample disbursement requests
INSERT INTO `disbursement_requests` (`id`, `request_number`, `requested_by`, `department`, `payee_name`, `payee_type`, `amount`, `purpose`, `payment_method`, `status`, `requested_at`, `approved_by`, `approved_at`, `paid_by`, `paid_at`) VALUES
(1, 'DR-2024-001', 'John Smith', 'Finance', 'ABC Office Supplies', 'Vendor', 25000.00, 'Office supplies and stationery for Q1', 'Bank Transfer', 'Approved', NOW(), 'Finance Manager', NOW(), 'Finance Team', NOW()),
(2, 'DR-2024-002', 'Maria Garcia', 'HR', 'Training Institute Inc.', 'Vendor', 50000.00, 'Employee training program fees', 'Bank Transfer', 'Pending', NOW(), NULL, NULL, NULL, NULL),
(3, 'DR-2024-003', 'Robert Johnson', 'IT', 'Tech Solutions Ltd.', 'Vendor', 75000.00, 'Software licensing renewal', 'Bank Transfer', 'Approved', NOW(), 'IT Director', NOW(), NULL, NULL);

-- Insert sample accounts payable
INSERT INTO `accounts_payable` (`id`, `vendor_name`, `invoice_number`, `invoice_amount`, `due_date`, `payment_terms`, `status`, `priority`, `description`, `created_by`, `created_at`) VALUES
(1, 'ABC Office Supplies', 'INV-2024-001', 25000.00, '2024-02-15', 'Net 30', 'approved', 'medium', 'Office supplies and stationery', 1, NOW()),
(2, 'Tech Solutions Ltd.', 'INV-2024-002', 75000.00, '2024-02-20', 'Net 15', 'pending', 'high', 'Software licensing renewal', 1, NOW()),
(3, 'Training Institute Inc.', 'INV-2024-003', 50000.00, '2024-02-25', 'Net 30', 'pending', 'medium', 'Employee training program fees', 1, NOW());

-- Insert sample accounts receivable
INSERT INTO `accounts_receivable` (`id`, `customer_name`, `customer_email`, `invoice_number`, `invoice_amount`, `due_date`, `payment_terms`, `status`, `priority`, `description`, `created_by`, `created_at`) VALUES
(1, 'XYZ Corporation', 'billing@xyzcorp.com', 'INV-2024-001', 150000.00, '2024-02-15', 'Net 30', 'pending', 'high', 'Q1 service fees', 1, NOW()),
(2, 'ABC Industries', 'accounts@abcind.com', 'INV-2024-002', 75000.00, '2024-02-20', 'Net 15', 'pending', 'medium', 'Consulting services', 1, NOW()),
(3, 'DEF Enterprises', 'finance@defent.com', 'INV-2024-003', 200000.00, '2024-02-25', 'Net 30', 'pending', 'high', 'Software implementation project', 1, NOW());

-- Insert sample collections
INSERT INTO `collections` (`id`, `collection_type`, `amount`, `reference_number`, `customer_name`, `customer_email`, `payment_method`, `status`, `collection_date`, `notes`, `created_by`, `commission_rate_id`, `commission_amount`, `commission_status`, `platform_id`, `platform_transaction_id`, `platform_reference`, `reconciliation_status`, `receipt_generated`, `region`, `auto_reconciled`, `created_at`) VALUES
(1, 'payment', 150000.00, 'COL-2024-001', 'XYZ Corporation', 'billing@xyzcorp.com', 'bank_transfer', 'completed', '2024-01-15', 'Q1 service fees payment', 1, 1, 7500.00, 'calculated', 5, 'TXN-001', 'REF-001', 'matched', 1, 'Metro Manila', 1, NOW()),
(2, 'invoice', 75000.00, 'COL-2024-002', 'ABC Industries', 'accounts@abcind.com', 'credit_card', 'completed', '2024-01-16', 'Consulting services payment', 1, 1, 3750.00, 'calculated', 8, 'TXN-002', 'REF-002', 'matched', 1, 'Cebu', 1, NOW()),
(3, 'payment', 200000.00, 'COL-2024-003', 'DEF Enterprises', 'finance@defent.com', 'bank_transfer', 'pending', '2024-01-17', 'Software implementation project payment', 1, 3, 15000.00, 'not_calculated', 6, 'TXN-003', 'REF-003', 'pending', 0, 'Davao', 0, NOW());

-- Insert sample commission calculations
INSERT INTO `commission_calculations` (`id`, `collection_id`, `commission_rate_id`, `base_amount`, `commission_amount`, `calculation_date`, `status`, `paid_date`, `notes`, `created_by`, `created_at`) VALUES
(1, 1, 1, 150000.00, 7500.00, '2024-01-15', 'approved', NULL, 'Standard 5% commission on payment', 1, NOW()),
(2, 2, 1, 75000.00, 3750.00, '2024-01-16', 'approved', NULL, 'Standard 5% commission on invoice', 1, NOW()),
(3, 3, 3, 200000.00, 15000.00, '2024-01-17', 'pending', NULL, 'High value 7.5% commission on payment', 1, NOW());

COMMIT;

-- ============================
-- AUTO_INCREMENT for new financial tables
-- ============================
...
--
-- AUTO_INCREMENT for table `disbursement_approvals`
--
ALTER TABLE `disbursement_approvals`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `collection_activities`
--
...
--
-- AUTO_INCREMENT for table `budget_approval_workflow`
--
ALTER TABLE `budget_approval_workflow`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `budget_management`
--
...
--
-- AUTO_INCREMENT for table `collection_analytics`
--
ALTER TABLE `collection_analytics`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `collection_receipts`
--
ALTER TABLE `collection_receipts`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `collection_refunds`
--
ALTER TABLE `collection_refunds`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `financial_statement_templates`
--
ALTER TABLE `financial_statement_templates`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
-- Note: HR compatibility view in main database removed for localhost
-- This view was causing cross-database reference issues
-- HR data should be accessed directly from hr_core database
...
-- =============================================================
-- Logistics 1 - Admin Side Additions (Company Assets & Requests)
-- =============================================================
USE `core_transaction2`;

-- Company assets master (internal belongings: PCs, chairs, desks, etc.)
CREATE TABLE IF NOT EXISTS `company_assets` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_company_assets_tag` (`asset_tag`),
  KEY `idx_company_assets_status` (`status`),
  KEY `idx_company_assets_wh_bin` (`warehouse_id`,`bin_id`),
  CONSTRAINT `fk_ca_wh` FOREIGN KEY (`warehouse_id`) REFERENCES `warehouses` (`id`),
  CONSTRAINT `fk_ca_bin` FOREIGN KEY (`bin_id`) REFERENCES `warehouse_bins` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Asset assignments (department/user issuance)
CREATE TABLE IF NOT EXISTS `asset_assignments_internal` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `asset_id` int(11) NOT NULL,
  ...
  CONSTRAINT `fk_assign_asset_internal` FOREIGN KEY (`asset_id`) REFERENCES `company_assets` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Maintenance history for company assets
CREATE TABLE IF NOT EXISTS `asset_maintenance_internal` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `asset_id` int(11) NOT NULL,
  ...
  PRIMARY KEY (`id`),
  KEY `idx_maint_asset` (`asset_id`),
  CONSTRAINT `fk_maint_asset_internal` FOREIGN KEY (`asset_id`) REFERENCES `company_assets` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Department asset requests (no justification per requirement)
CREATE TABLE IF NOT EXISTS `asset_requests` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  ...
  PRIMARY KEY (`id`),
  KEY `idx_asset_requests_status` (`status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE IF NOT EXISTS `asset_request_approvals` (
  ...
  PRIMARY KEY (`id`),
  KEY `idx_asset_request_approvals_req` (`request_id`),
  CONSTRAINT `fk_ar_approval_request` FOREIGN KEY (`request_id`) REFERENCES `asset_requests` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

-- Seed: example company assets (safe to run multiple times)
INSERT INTO `company_assets` (`asset_tag`,`name`,`category`,`status`,`purchase_date`,`cost`,`notes`) VALUES
  ('IT-CPU-0001','Desktop Unit - Dell OptiPlex 7090','Computer','Available','2024-03-10',65000.00,'i7/16GB/512GB NVMe'),
  ('IT-MON-0001','Monitor - Dell 24" IPS','Monitor','Available','2024-03-10',12000.00,'1080p'),
  ('IT-KBD-0001','Keyboard - Logitech K120','Peripherals','Available','2024-03-12',650.00,NULL),
  ('IT-MSE-0001','Mouse - Logitech B100','Peripherals','Available','2024-03-12',450.00,NULL),
  ('IT-HDP-0001','Headset - HyperX Cloud Stinger','Peripherals','In Use','2024-05-02',2950.00,NULL),
  ('IT-PSU-0001','UPS - APC 1500VA','Electrical','In Use','2024-04-18',14500.00,'For server workstation'),
  ('NET-SW-0001','Network Switch - Cisco SG350-28','Networking','Available','2023-11-05',38500.00,'Managed Gigabit'),
  ('NET-AP-0001','WiFi Access Point - Ubiquiti UAP-AC-PRO','Networking','In Use','2023-12-01',9200.00,NULL),
  ('OF-FUR-0001','Office Chair - Ergonomic Mesh','Furniture','Available','2024-01-20',4800.00,NULL),
  ('OF-TBL-0001','Office Table - 160x80cm','Furniture','Available','2024-01-20',5200.00,NULL),
  ('WH-TRK-0001','Hand Pallet Truck - 2.5T','Warehouse Equipment','Maintenance','2023-09-15',18500.00,'Wheels replacement scheduled'),
  ('WH-SHL-0001','Metal Shelving Rack - 5 Tier','Warehouse Equipment','Available','2023-08-10',7800.00,NULL),
  ('VEH-TRN-0001','Delivery Van - Toyota HiAce','Vehicle','In Use','2022-06-01',1450000.00,'Plate ABC-1234'),
  ('PWR-GEN-0001','Diesel Generator - 20kVA','Power','Maintenance','2021-10-12',325000.00,NULL),
  ('PROP-LND-0001','Parcel of Land - Warehouse Lot','Property','Available',NULL,0.00,'Title T-001234 (asset control)'),
  ('PROP-BLD-0001','Warehouse Building - Main','Property','In Use',NULL,0.00,'Depreciation schedule maintained');

-- Additional diverse seed assets (30+ items)
INSERT INTO `company_assets` (`asset_tag`,`name`,`category`,`status`,`purchase_date`,`cost`,`notes`) VALUES
  ('IT-LPT-0001','Laptop - Lenovo ThinkPad T14','Computer','In Use','2024-02-14',84000.00,'Ryzen 7/16GB/512GB'),
  ('IT-PRN-0001','Printer - HP LaserJet Pro M404dn','Peripherals','Available','2023-10-05',14500.00,'Duplex'),
  ('IT-SCN-0001','Scanner - Epson DS-530','Peripherals','Available','2023-09-22',18500.00,NULL),
  ('IT-SRV-0001','Server - HPE ProLiant DL380','Computer','In Use','2023-07-30',450000.00,'VM host'),
  ('IT-NAS-0001','NAS - Synology DS920+','Storage','In Use','2023-08-12',52000.00,'4x6TB RAID5'),
  ('SEC-CAM-0001','CCTV Camera - Dome 4MP','Security','In Use','2023-05-03',3800.00,NULL),
  ('SEC-DVR-0001','NVR - 16 Channel','Security','In Use','2023-05-03',18500.00,NULL),
  ('TEL-PBX-0001','IP PBX - Grandstream UCM6204','Telecom','In Use','2023-04-18',24500.00,NULL),
  ('TEL-PHN-0001','IP Phone - Yealink T31G','Telecom','Available','2023-04-20',3200.00,NULL),
  ('CLN-VAC-0001','Industrial Vacuum','Cleaning','Available','2023-08-18',9500.00,NULL),
  ('CLN-MOP-0001','Auto Floor Scrubber','Cleaning','Maintenance','2023-06-25',48500.00,NULL),
  ('SAF-EXT-0001','Fire Extinguisher - ABC 10lb','Safety','In Use','2024-01-05',1800.00,'Next service: 2025-01'),
  ('SAF-AED-0001','AED - Defibrillator','Safety','In Use','2022-12-19',75000.00,NULL),
  ('MED-KIT-0001','First Aid Cabinet','Safety','In Use','2024-02-01',3500.00,NULL),
  ('PWR-INV-0001','Inverter - 3kW','Power','Available','2023-11-30',28500.00,NULL),
  ('PWR-BAT-0001','Battery Bank - 200Ah','Power','Available','2023-12-02',16500.00,NULL),
  ('HVAC-AC-0001','Air Conditioner - 2.5HP Split','HVAC','In Use','2023-03-22',38500.00,NULL),
  ('HVAC-FAN-0001','Industrial Exhaust Fan 24"','HVAC','In Use','2023-03-15',6800.00,NULL),
  ('TOOL-DRL-0001','Cordless Drill Set','Tools','Available','2024-02-10',5200.00,NULL),
  ('TOOL-WLD-0001','Arc Welder 200A','Tools','Available','2023-07-07',18500.00,NULL),
  ('MAT-PAL-0001','Plastic Pallets - Set of 20','Materials','Available','2023-09-12',24000.00,NULL),
  ('MAT-CRT-0001','Crates - Foldable (50 pcs)','Materials','Available','2023-09-12',15000.00,NULL),
  ('IT-CBL-0001','Ethernet Cable Drum - 305m Cat6','Consumables','Available','2024-03-01',5200.00,NULL),
  ('IT-SRG-0001','Surge Protector - 8 Outlets','Electrical','Available','2024-01-18',950.00,NULL),
  ('LGT-LED-0001','LED High Bay Light 150W','Lighting','In Use','2023-10-28',4200.00,NULL),
  ('PKG-SCL-0001','Digital Weighing Scale - 150kg','Packaging','In Use','2023-05-20',8600.00,NULL),
  ('PKG-STR-0001','Strapping Machine - Semi-auto','Packaging','In Use','2023-05-20',48500.00,NULL),
  ('MOB-FRK-0001','Forklift - 3.5T LPG','Vehicle','In Use','2021-04-09',820000.00,'Monthly PM'),
  ('MOB-PCK-0001','Pickup Truck - Hilux','Vehicle','In Use','2022-02-14',1120000.00,'Plate XYZ-5678'),
  ('PROP-PRK-0001','Parking Lot - Leasehold','Property','In Use',NULL,0.00,'20 slots');
